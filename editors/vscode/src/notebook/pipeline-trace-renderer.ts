...
export function activate(): {
  renderOutputItem(outputItem: any, element: any): void;
} {
  return {
    renderOutputItem(outputItem: any, element: any): void {
...
function renderTable(outputItem: any, element: any): void {
...
function renderHtml(outputItem: any, element: any): void {
  const payload = outputItem.json();
...
function renderPipelineTrace(outputItem: any, element: any): void {
  const payload = outputItem.json();
  ...
  stages.forEach((stage: any, index: number) => {
    flow.appendChild(
      renderStageCard(stage, payload, detailHost, (stageName: string) => {
...
function renderStageDetail(payload: any, element: any, stageName: string | null): void {
  clearElement(element);
  if (!stageName) {
    return;
  }
  ...
  const recordsByStage =
    payload.records_by_stage && typeof payload.records_by_stage === "object"
...
function applyCellStyle(cell: any, header: boolean): void {
...
function renderSourceLines(payload: any, container: any): void {
  const snippet = typeof payload.source_snippet === "string" ? payload.source_snippet : "";
  const lines = snippet.split(/\r?\n/);
  const corruptedLines = new Map<number, number>();
  const stages = Array.isArray(payload.stages) ? payload.stages : [];
...
function firstCorruptedStage(payload: any): string | null {
  const stages = Array.isArray(payload.stages) ? payload.stages : [];
  const found = stages.find((stage: any) => numberValue(stage.corrupted) > 0);
  return found ? stringValue(found.name) : null;
}

function inferObjectColumns(rows: any[]): string[] {

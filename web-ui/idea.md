...
- コンポーネントは `.bloom` ファイル単位（Vue SFCに近い）
...
- `{count}` — テンプレート補間
- `@click={fn}` — 関数参照。クロージャ（`e => ...` / `fn(e) { ... }`）もインラインで記述可
- `class="..."` — Tailwindユーティリティクラスをそのまま使用

### フォームモデル分離パターン（`*.model.bloom`）

`v-model` 相当の双方向バインディングは、モデルを専用ファイルに分離する方式を採用する。
`layout.bloom` と同じ命名規約で、フォーム状態・バリデーション・送信ロジックを責務分離する。

```
src/app/users/new/
  page.bloom            ← ビュー（テンプレート + UIイベント）
  page.model.bloom      ← モデル（フォーム状態 + バリデーション + 送信）
```
...
- `:bind={field}` — 双方向バインディング（値の読み取り + 入力イベントの更新を自動接続）
- モデルはテスト可能な純粋なロジック。テンプレートと完全に分離される
...
|-----|-------|
| `.vue` SFC | `.bloom` SFC |
| `<script setup>` | `<script>` in `.bloom` |
| `v-model` | `*.model.bloom`（モデルファイルに分離） + `:bind={model.field}` |
| `@click` | `@click={fn}` または `@click={e => ...}` |
| Vite | `forge dev` |
...
Storybookを外部ツールとして入れる代わりに、`.bloom`ファイルに直接書ける。
...
| イベントバインディング | `@click={fn}` または `@click={e => ...}` / `@click={fn(e) { ... }}`。関数参照・クロージャ両方可 |
| 双方向バインディング | `v-model` に相当する機能は `*.model.bloom` ファイルに分離。テンプレートでは `:bind={model.field}` で接続 |
| モデルファイル規約 | `page.model.bloom` — フォーム状態・バリデーション・送信ロジックを担う。`layout.bloom` と同じ命名規約 |
| コンポーネント呼び出し | PascalCaseタグ（`<Counter />`）。HTMLタグはすべて小文字なので衝突しない |
| 拡張子 | `.bloom`（コンポーネント）/ `.flux.bloom`（ストア）/ `.model.bloom`（フォームモデル） |
| Islands ロード戦略 | `@island(load: "immediate")` / `@island(load: "visible")` / `@island(load: "idle")`。コンポーネント単位で指定。デフォルトは `immediate` |
| リポジトリ配置 | `packages/bloom/`（このリポジトリ内） |
...
`forge new my-app --bloom` で生成されるプロジェクトは最初から Bloom + Tailwind が設定済み。
...
└── src/
    ├── app/
    │   ├── layout.bloom       ← <div class="p-4 text-blue-500"> がすぐ動く
    │   └── page.bloom
    ├── components/
...
**`@island` ロード戦略（コンポーネント単位で指定）:**

```bloom
@island                         // デフォルト = immediate（省略可）
@island(load: "immediate")      // ページロード時に即時ロード
@island(load: "visible")        // viewport進入時にロード
@island(load: "idle")           // ブラウザidle時にロード
```

`forge.min.js` がスクロール・IntersectionObserver・requestIdleCallback を使い分けて各 Island を遅延ロードする。
...
| **B-3** | SSR + 全置換アタッチ | Anvil が HTML 出力 → WASM が `innerHTML` 相当で全置換 → チラつきなし確認 |
| **B-4** | DOM Morphing（差分化） | 全置換を差分 Morph に置き換え → フォーカス・入力状態を保持したまま更新 |
| **B-5** | `typestate` ストア + DevTools | ステートマシングラフ可視化・タイムトラベルデバッグ |
| **B-6** | プロジェクトスキャフォールド | `forge new --bloom` + `forge bloom add` CLI |

**B-0 から始める理由**: ブリッジがなければ B-1 以降はすべてデッドコード。
B-0 が完成した時点で「ForgeScript から Hello World をブラウザに表示する」という最初のマイルストーンが達成できる。

**B-3 → B-4 の 2 段構えの理由**: DOM Morphing のエッジケース（フォーカス・テキスト入力中・アニメーション・カスタム要素）は実装コストが高い。B-3 の全置換で SSR アタッチ全体が動くことを先に確認してから、B-4 で段階的に品質を上げる。
...
> **参照ファイル:**
> - 実装プロトタイプ（Next.js / TypeScript）: `web-ui/bloon-ts/`
> - ビジュアルイメージ: `web-ui/bloom-image.png`

### レイアウト構成

```
┌─────────────────────────────────────────────────────┐
│ 📄 Get started by editing src/app/page.bloom  GitHub│  ← ヘッダー
├─────────────────────────────────────────────────────┤
│                                                      │
│                                                      │
│                      Bloom                          │  ← ロゴ（グラデーション）
│                 on ForgeScript                      │  ← サブタイトル
│                                                      │
│                                                      │
├─────────────────────────────────────────────────────┤
│  Docs →          Learn →         Templates →        │  ← フッターカード3枚
│  Bloomの機能と    インタラクティブ  スターター          │
│  APIの詳細        なコースで学ぶ   テンプレート集       │
└─────────────────────────────────────────────────────┘
```
...
| 要素 | 仕様 |
|---|---|
| 背景 | ピュアブラック（ダーク固定） `oklch(0.145 0 0)` |
| フォント | Geist / Geist Mono |
| ロゴ "Bloom" | `text-7xl font-bold` / グラデーション `from-rose-400 via-fuchsia-500 to-indigo-500` |
| サブタイトル | `on ` (muted) + `ForgeScript` (bold / white) |
| フッターカード | `border-border` / hover で `border-foreground/20 + bg-secondary/50` |
| ヘッダー左 | FileCode アイコン + ファイル名をコードブロック表示 |
| ヘッダー右 | GitHub リンク + GitHub アイコン |

### Next.js との差分

| 要素 | Next.js | Bloom |
|---|---|---|
| 背景 | ライトグレー（グラデーション） | **ピュアブラック（ダーク固定）** |
| ロゴ色 | モノクロ | **ピンク→パープルのグラデーション** |
| ロゴ下テキスト | なし | **on ForgeScript** |
| 右上 | By ▲ Vercel | **GitHub** |
| フッターリンク | Docs / Learn / Templates / Deploy | **Docs / Learn / Templates**（Deploy なし） |

### 実装上の注意

- このページ自体を `.bloom` で実装することで「Bloom で Bloom の顔を作る」ドッグフーディングになる
- B-1 フェーズ完了後、`web-ui/bloon-ts/app/page.tsx` を `.bloom` に移植する
- `layout.tsx` の `<html className="dark">` に相当する設定は `forge.toml` の `[bloom]` セクションで管理する
...
| 拡張子 | 種別 | 説明 |
|---|---|---|
| `.forge` | ForgeScript | 純粋な ForgeScript（型定義・ユーティリティ・ストア内ロジック等） |
...
`.bloom` / `.flux.bloom` はともに ForgeScript ランタイム上で動作する。

---

## プロジェクト構造

### `forge new` のテンプレートフラグ

`forge new` は将来複数のテンプレートをサポートするため、フラグで種別を指定する。

```bash
forge new my-app                  # 素の ForgeScript プロジェクト
forge new my-app --bloom          # Bloom フロントエンド
forge new my-app --anvil          # Anvil バックエンド
forge new my-app --fullstack      # Anvil + Bloom フルスタック
```

### `forge new my-app --bloom` が生成するディレクトリ構造

```
my-app/
├── forge.toml                    ← [bloom] tailwind = true が設定済み
├── public/
│   └── favicon.ico
└── src/
    ├── app/                      ← ルーティング（URL 構造と 1:1）
    ...
    ├── stores/                   ← 状態管理
    │   └── counter.flux.bloom    ← サンプルストア
    └── lib/                      ← 共有ロジック（純粋 ForgeScript）
        └── utils.forge
```

Next.js App Router との主な違い:

| | Next.js | Bloom |
|---|---|---|
| API ルート | `app/api/route.ts` | **なし**（Anvil が担当） |
| 状態管理 | 慣習任せ | `stores/*.flux.bloom`（公式） |
| 設定ファイル | `next.config.js` | `forge.toml` の `[bloom]` セクション |
| 共有ロジック | `lib/*.ts` | `lib/*.forge` |
...
## `forge bloom add` — スキャフォールド CLI

既存の `forge notebook` / `forge goblet` と同じく `forge` のサブコマンドとして統合。

```bash
# コンポーネント → src/components/<name>.bloom
forge bloom add component button

# ページ → src/app/<path>/page.bloom（ディレクトリも自動作成）
forge bloom add page about
forge bloom add page users/[id]

# レイアウト → src/app/<path>/layout.bloom
forge bloom add layout admin

# ストア → src/stores/<name>.flux.bloom
forge bloom add store cart
```

各コマンドが生成するボイラープレート:

```bloom
{{! forge bloom add component button → src/components/button.bloom }}
...
```bloom
{{! forge bloom add page about → src/app/about/page.bloom }}
<div class="p-4">
...
```
{{! forge bloom add store cart → src/stores/cart.flux.bloom }}
store Cart {
  state items: list<string> = []

  fn add(item: string) { items = items.push(item) }
  fn clear() { items = [] }
}
```
...
| B-3（SSR 全置換） | REPLACE_INNER / ATTACH（既存 DOM ノードへの参照取得） |
| B-4（Morphing） | MORPH_NODE / MOVE_NODE / PATCH_ATTRS（差分更新用） |
...
Rust クレートは `crates/bloom/` として追加する。

...
### 位置づけ：AI と一緒に探索するセキュアなサンドボックス

Jupyter との根本的な違いはここにある。

```
Jupyter:
  人間がコードを書く → 実行する → 結果を見る
  AI は外部ツールとして後付けで統合

ForgeScript Notebook:
  AI と人間が一緒にコードを探索する場所
  MCP が最初から統合されている
  WASM サンドボックスで安全に実行される
```

Web における WASM のような「セキュアな実行環境」として設計する。
数学・ML・物理演算・論文検証が ForgeScript 一本で動き、
AI が文脈を理解した状態で隣にいる。
...
- **WASM サンドボックス**: セルは `WasmOptions::sandboxed()` 相当の制約下で実行
  - 無限ループ → タイムアウトで自動停止（「カーネルを殺す」体験なし）
  - メモリ上限超過 → 即座に停止
  - ファイル・ネットワークアクセス → ポリシーで制御
- **MCP ファースト**: `forge-mcp` が起動中であれば AI との対話がセル内から直接できる
...
`forge-mcp` が起動中であれば、ノートブックセル内から直接 AI に質問・依頼できる。
...
// @ai: この関数の変曲点を求めるコードを書いてください
// → AI がコードを生成してセルの次の行に挿入

let inflection = f.diff("x", order: 2).solve("x")
display::math(inflection)
...
コンパイルエラー・実行時エラーが出た場合、VS Code の拡張機能が
"AI に説明を聞く" ボタンを表示する。クリックすると MCP 経由でエラーコンテキストを
AI に送り、修正提案をセル直下に表示する。
...
「コードを書く場所」ではなく「AI と一緒に数学・データを探索する場所」として機能する。

### セキュリティ：WASM サンドボックス実行

各セルは `WasmOptions::sandboxed()` 相当の制約下で実行される。

```
無限ループ       → タイムアウト（500ms）で自動停止
メモリ超過       → 即座に停止（16MB 上限）
FS アクセス      → ポリシーで制御（デフォルト: プロジェクトディレクトリのみ）
ネットワーク     → ポリシーで制御（デフォルト: 無効）
```

Jupyter の「カーネルが止まらなくて強制終了」という体験がなくなる。
...
| **N-5: 可視化** | `display::plot` + `forge/std/plot` / `forge/std/ml` 統合 |
| **N-6: MCP + AI 統合** | `@ai:` コメント構文・エラー時 AI サポート・MCP 自動起動 |
| **N-7: WASM サンドボックス** | セル実行を `WasmOptions::sandboxed()` で隔離・タイムアウト・メモリ上限 |

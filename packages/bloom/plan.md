...
1. **`.bloom` パーサー（`crates/bloom-compiler/src/parser.rs`）** ← Rust（必須）
   - テンプレート部分（HTML + `{expr}` 補間）の AST 生成
   - `<script>` ブロックの ForgeScript コードを `forge-compiler` に委譲
   - `@event={handler}` 属性の解析
   - `:attr={expr}` 属性の解析

2. **コード生成（`crates/bloom-compiler/src/codegen.rs`）** ← Rust（必須）
   - テンプレート AST → DOM コマンド列を発行する ForgeScript コードに変換
   - `state` 変数の変更を検知してコマンドを発行するコードを自動生成
   - `@click` → `ADD_LISTENER` コマンドに変換

3. **`forge build --web` への統合**
   - `.bloom` ファイルを検出してパース
   - 生成コードを WASM にコンパイル
...
1. **依存解析（`crates/bloom-compiler/src/reactivity.rs`）** ← Rust（コンパイラ拡張）
   - テンプレート中で `state` 変数を参照しているノードを静的に収集
   - `{count}` → `<p>` の `textContent` が `count` に依存、と記録

2. **更新コード生成（同上）** ← Rust（コンパイラ拡張）
...
| クレート / パッケージ | 種別 | 用途 |
|---|---|---|
| `packages/bloom/src/` | ForgeScript | フレームワーク本体（dom / ssr / router / store / morph） |
| `crates/bloom-compiler/` | Rust | .bloom パーサー・コード生成・リアクティビティ解析 |
| `forge-compiler` | Rust（既存） | AST / パース（委譲先） |
| `forge-vm` | Rust（既存） | コード実行 |
| `serde` / `serde_json` | Rust（既存） | SSR・DevTools データのシリアライズ |
